...
char const *const LCD_FILE           = "/sys/class/leds/lcd_backlight0/brightness";
...
static int write_leds_locked(struct led_config *led) {
    ...
    write_int(RED_DELAY_ON_FILE, led->onMS);
    write_int(RED_DELAY_OFF_FILE, led->offMS);

    write_int(GREEN_DELAY_ON_FILE, led->onMS);
    write_int(GREEN_DELAY_OFF_FILE, led->offMS);
    
    write_int(BLUE_DELAY_ON_FILE, led->onMS);
    write_int(BLUE_DELAY_OFF_FILE, led->offMS);
